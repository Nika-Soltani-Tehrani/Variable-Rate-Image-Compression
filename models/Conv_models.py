...
class ConvolutionalRecursiveCore(nn.Module):
    ...
    def __init__(self, coded_size=8, patch_size=32, batch_size=4, repetition=32):
...
class ResidualConvolutional(nn.Module):
    def __init__(self, coded_size=32, patch_size=32, repetition=32):
        super(ResidualConvolutional, self).__init__()
        self.repetition = repetition

        self.encoders = nn.ModuleList([ConvolutionalEncoder(coded_size, patch_size) for i in range(repetition)])
        self.decoders = nn.ModuleList([ConvolutionalDecoder(coded_size, patch_size) for i in range(repetition)])
